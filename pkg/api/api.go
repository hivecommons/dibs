...
// API wires the store and registry into HTTP handlers.
type API struct {
	Store    *store.Store
...
// loadAuthorized fetches an idea and enforces read access: the author always;
// anyone authenticated if the idea is public. Write access (mustOwn) is
// author-only. A private idea is presented to non-authors as 404, not 403,
// so its very existence doesn't leak.
